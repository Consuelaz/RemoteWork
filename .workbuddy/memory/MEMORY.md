...
### V2EX 详情页抓取（2026-03-24 增强）
- 抓取列表页后，预下载前7个岗位的详情页
- 解析详情页提取：岗位职责、任职要求、福利待遇、公司背景、薪资、工作方式
- 通过 URL 映射解决列表索引与详情文件不匹配问题
...
- 第2行：表头 [公司, 行业, 职位类别, 职位名称, 地区, 申请链接]
...
- **代码位置**：`app.js` 第115-119行 `formatCompany()` 函数、第267行详情页公司显示
