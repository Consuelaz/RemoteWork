...
## 最近一次执行: 2026-08-14 15:55

**结果**: ✅ 成功（手动触发，7892→8001 Python TCP 转发代理）
- Remote OK: 100 条 ✅
- Remotive: 18 条 ✅
- 远程岛: 150 条 ✅
...
- **汇总**: CN=1120, Global=2000
- **数据变化**: CN 1110→1120 (+10), Global 2000 (不变)
- **Git**: commit 73e0e9a，push 成功（b895f63..73e0e9a）
- **money.xlsx**: 更新成功（1条V2EX + 431条who-is-hiring）
- **代理**: 7892/7897均无进程监听，v2ray 8001可用，启动 Python TCP 转发 7892→8001；任务结束后转发已停止
- **文件大小**: jobs-cn.js 1.5MB, jobs-global.js 2.9MB, money.xlsx 8.3MB
- **备注**: 所有数据源均正常抓取

## 历史执行: 2026-08-12 14:00

**结果**: ✅ 成功（自动触发，7892→8001 Python TCP 转发代理）
- Remote OK: 100 条 ✅
- Remotive: 20 条 ✅
- 远程岛: 150 条 ✅
- who-is-hiring: 431 条（含联系方式 431 条）✅
...
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1110, Global=2000
- **数据变化**: CN 1092→1110 (+18), Global 2000 (不变)
- **Git**: commit b895f63，push 成功（4fe44a3..b895f63）
- **money.xlsx**: 更新成功（2条V2EX + 431条who-is-hiring）
- **代理**: 7892/7897端口均无进程监听；v2ray 在 8001 端口可用，启动 Python TCP 转发 7892→8001；任务结束后转发已停止
- **文件大小**: jobs-cn.js 1.5MB, jobs-global.js 2.9MB, money.xlsx 7.6MB
- **备注**: 所有数据源均正常抓取

## 历史执行: 2026-08-07 16:26
...
- V2EX: 2 条新帖（跳过 18 个非今日帖子）✅
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1092, Global=2000
- **数据变化**: CN 1072→1092 (+20), Global 2000 (不变)
- **Git**: commit 4fe44a3，push 成功（f2becac..4fe44a3）
- **money.xlsx**: 更新成功（2条V2EX + 431条who-is-hiring）
- **代理**: 7892/7897端口均无进程监听；v2ray 在 8001 端口可用（HTTP proxy），启动 Python TCP 转发 7892→8001；任务结束后转发已停止
- **备注**: 所有数据源均正常抓取，远程岛/远程中文网本次恢复正常

## 历史执行: 2026-07-31 09:32

**结果**: ✅ 成功（手动触发，7892→7897 Python 转发代理）
- Remote OK: 100 条 ✅
- Remotive: 35 条 ✅
- 远程岛: 0 条 ❌（5页JSON均为空，解析失败）
- who-is-hiring: 431 条（含联系方式 431 条）✅
- 远程中文网: 0 条 ❌（列表页解析为0条）
- V2EX: 0 条（跳过20个非今日帖子，今日无新帖）
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1072, Global=2000
- **数据变化**: CN 1070→1072 (+2), Global 2000 (不变)
- **Git**: commit f2becac，push 成功（6359cd6..f2becac）
- **money.xlsx**: 更新成功（0条V2EX + 431条who-is-hiring）
- **代理**: 7892端口无进程监听，启动 Python TCP 转发 7892→7897；任务结束后转发已停止
- **备注**: 远程岛/远程中文网本次异常返回空数据（与7月15日以来多次现象一致）；V2EX 今日无新帖（跳过20个非今日帖）

## 历史执行: 2026-07-27 12:24
...
- who-is-hiring: 431 条（含联系方式 431 条）✅
- 远程中文网: 0 条 ❌（列表页解析为0条）
- V2EX: 2 条新帖（跳过18个非今日帖子）✅
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1070, Global=2000
- **数据变化**: CN 1064→1070 (+6), Global 2000 (不变)
- **Git**: commit 6359cd6，push 成功（1347126..6359cd6）
- **money.xlsx**: 更新成功（2条V2EX + 431条who-is-hiring）
- **代理**: 7892端口无进程监听，启动 Python TCP 转发 7892→7897；任务结束后转发已停止
- **备注**: 远程岛/远程中文网本次异常返回空数据，与7月15日现象一致
...
- **数据变化**: CN 1058→1064 (+6), Global 2000 (不变)
- **Git**: commit 1347126，push 成功（8c85b6f..1347126）
- **money.xlsx**: 更新成功（2条V2EX + 431条who-is-hiring）
- **代理**: 7892端口无进程监听，临时改PROXY为7897运行，完成后改回7892；7897端口有verge-mih监听可用
- **备注**: 远程岛/远程中文网本次异常返回空数据

**结果**: ✅ 成功（手动触发，修复 IndentationError 后重新运行）
- Remote OK: 100 条 ✅
- Remotive: 27 条 ✅
- 远程岛: 150 条 ✅（恢复）
- who-is-hiring: 431 条（含联系方式 431 条）✅
- 远程中文网: 15 条 ✅（IndentationError 修复后恢复）
- V2EX: 0 条 ❌（代理7892/7897均不可用，直连也不通）
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1058, Global=2000
- **数据变化**: CN 1042→1058 (+16), Global 1947→2000 (+53)
- **Git**: commit 8c85b6f，push 成功（7a8f9f2..8c85b6f）
- **money.xlsx**: 更新成功（0条V2EX + 431条who-is-hiring）
- **修复**: scrape.sh 远程中文网段落 IndentationError（url_id/job缩进未进入if块），已修复
- **代理**: 7892/7897均无进程监听，V2EX不可用

## 历史执行: 2026-07-08 12:07

**结果**: ✅ 成功（手动触发，7892→7897 转发代理）
- Remote OK: 100 条 ✅
- Remotive: 28 条 ✅
- 远程岛: 0 条 ❌（5页JSON均为空，解析失败）
- who-is-hiring: 431 条（含联系方式 431 条）✅
...
- V2EX: 2 条新帖（跳过18个非今日帖子）✅
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1101, Global=2000
- **数据变化**: CN 1097→1101 (+4), Global 2000 (不变)
- **Git**: commit e5c9ea7，push 成功（655b35d..e5c9ea7）
- **money.xlsx**: 更新成功（2条V2EX + 431条who-is-hiring）
- **代理**: 7892原生不可用，启动7892→7897 TCP转发；任务结束后转发已停止
- **备注**: 远程岛/远程中文网本次异常返回空数据

## 历史执行: 2026-07-07 11:30
...
- who-is-hiring: 431 条（含联系方式 431 条）✅
- 远程中文网: 0 条 ❌（列表页解析为0条）
- V2EX: 3 条新帖（跳过17个非今日帖子）✅
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1097, Global=2000
- **数据变化**: CN 1094→1097 (+3), Global 2000 (不变)
- **Git**: commit 655b35d，push 成功（2f347e7..655b35d）
- **money.xlsx**: 更新成功（3条V2EX + 431条who-is-hiring）
- **代理**: 7892原生不可用，启动7892→7897 TCP转发；任务结束后转发已停止
- **备注**: 远程岛/远程中文网本次异常返回空数据

## 历史执行: 2026-07-06 13:04

**结果**: ✅ 成功（手动触发，通过 7892→7897 转发代理修复）
- Remote OK: 100 条 ✅
- Remotive: 30 条 ✅
- 远程岛: 0 条 ❌（5 页 JSON 均为空，解析失败）
- who-is-hiring: 431 条（含联系方式 431 条）✅
- 远程中文网: 0 条 ❌（列表页解析为 0 条）
- V2EX: 1 条新帖（跳过 19 个非今日帖子）✅
- 电鸭: 跳过（文件不存在 /tmp/dianyu.html）
- **汇总**: CN=1094, Global=2000
- **数据变化**: CN 1093→1094 (+1), Global 2000 (不变)
- **Git**: commit d5b73c4，push 成功（a781984..d5b73c4）
- **money.xlsx**: 更新成功（1条V2EX + 431条who-is-hiring）
- **代理**: 127.0.0.1:7892 原生不可用，已启动 7892→7897 TCP 转发使 scrape.sh 可用；任务结束后转发已停止
- **备注**: V2EX 恢复抓取；远程岛/远程中文网本次异常返回空数据
- **文件大小**: jobs-cn.js 1.5MB, jobs-global.js 3.2MB, money.xlsx 5.6MB
